...
async function terminate(driver, url) {
    ...
    try {
        await driver
            .findElement(
                By.xpath(`//*[@id="dashboardTabs-tabpane-clusters"]/div[1]/div[1]/div[2]/div/div[1]/div[2]/div[3]/div[2]/div/div/div/div[8]/div/div/span[2]/svg[2]`)
            )
            .click(); //Terminate Server
        console.log("Finally Success!!!"); //To check the code
        await driver.sleep(5000); // time 5secs
    } catch (error) {
    ...
    // Accept the Action
    try {
        await driver.switchTo().alert().accept(); //Accept the action
